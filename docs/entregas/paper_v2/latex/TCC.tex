...
Este trabalho propõe a adaptação do \textit{Ralph Wiggum Loop}, paradigma de orquestração originalmente concebido para fluxos focados em desenvolvimento e engenharia de software e que traz soluções para mitigar problemas de \textit{context rot}, para um fluxo de resolução generalista, focado em responder perguntas em linguagem natural. A hipótese é que essa estrutura melhora a qualidade das respostas em relação à inferência singular, e essa hipótese é avaliada por meio dos \textit{benchmarks} MMLU e GPQA, comparando chamadas diretas ao modelo com execuções mediadas pelo \textit{harness} proposto, utilizando modelos da família Llama~3.1 com diferentes quantidades de parâmetros. A acurácia, medida pela correspondência entre a alternativa selecionada pelo sistema e o gabarito oficial, é adotada como métrica principal.}
...
\begin{multline*}
         	ext{\textit{Contexto Completo da Tarefa}} \\
  + \text{\textit{Template B.4}} \\
...
Para contextualizar os resultados obtidos em relação ao estado da arte, os valores de acurácia dos modelos Llama~3.1 nas duas configurações são comparados com os resultados de inferência direta já publicados para Claude Sonnet~4.6 e GPT~5.4 nos mesmos \textit{benchmarks}. Esses modelos comerciais não são submetidos ao \textit{harness} proposto: sua função é exclusivamente estabelecer um ponto de referência de mercado que permita situar o desempenho alcançado pela abordagem proposta em relação a sistemas modernos de grande escala.
