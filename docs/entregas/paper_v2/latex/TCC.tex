...
O planejamento é o início de todo o fluxo. Aqui assumimos que estamos recebendo uma pergunta e vamos iniciar o fluxo de \textit{Thinking} da aplicação. Assim como exemplificado na Figura~\ref{fig:ralph_loop_adaptado}, o planejamento possui somente duas etapas.

Como referência para os exemplos práticos desta seção, adota-se o seguinte input:
...
- Explicação da responsabilidade:
  - Essa é uma fase que não era necessária no contexto de desenvolvimento de software, pois a cada tarefa entregue um commit era realizado salvando o progresso, e quando todas as tarefas estivessem encerradas, o desenvolvimento também estaria pronto.
  - Agora o cenário muda: após todas as tarefas serem concluídas, será necessário enviar uma resposta final consolidando os resultados gerados. Esses critérios definem as regras que a resposta final deverá respeitar.
    - Serão definidos pontos como: idioma a ser utilizado, formato de apresentação esperado e tópicos-chave que devem constar na resposta final.

- Como vai ser feito:
  - Criação de um template FIXO, onde será injetado o input passado pelo usuário. Esse template deve conter um prompt solicitando a definição de uma lista de critérios de aceite que serão resgatados na fase de Criação da Resposta Final.
  - O template solicita a definição de:
    - Idioma a ser usado na resposta final
    - Formato em que a resposta deve ser construída
    - Tópicos-chave do conteúdo que devem estar presentes na resposta final

- Equação de funcionamento da fase:
  - Pergunta do usuário + Template A.1 = Resultado Fase A.1
  - O Resultado Fase A.1 deve ser armazenado localmente em arquivo ou na memória do programa.

- Exemplo prático:
  - Assumindo o input de referência, o LLM deveria retornar algo como:
...
- Explicação da responsabilidade:
  - Quebrar a pergunta do usuário em pequenas tarefas autônomas.
  - Cada tarefa precisa ter pelo menos um critério de aceite, isto é, o conteúdo esperado para que a tarefa possa ser considerada ``Concluída''.
  - A ideia é criar tarefas que gerem insumos capazes de, em conjunto, sustentar a construção da resposta final.

- Como vai ser feito:
  - Criação de um template FIXO, onde será injetado o input passado pelo usuário, solicitando sua quebra em tarefas.
    - O template deve especificar que tarefas de objetivo único possuem uma única responsabilidade pontual — como ``identificar os números primos'' ou ``identificar os números pares'' — mas nunca ``identificar primos, verificar paridade e comparar com 2 simultaneamente''.
    - Além de ter um único objetivo, a tarefa deve ser a mais detalhada possível para evitar desvios.
    - Deve ficar claro que o contexto de execução de uma tarefa não é transferido para a próxima; somente pontos específicos sintetizados como aprendizados pontuais serão preservados.
    - Toda tarefa deve ter pelo menos um critério de aceite que valide se a resposta gerada contém o conteúdo necessário para encerrá-la.

- Equação de funcionamento da fase:
  - Pergunta do usuário + Template A.2 = Resultado Fase A.2
  - O Resultado Fase A.2 deve ser armazenado localmente em arquivo ou na memória do programa.
...
- Exemplo prático:
  ...
  \textit{Nota: o formato acima é meramente ilustrativo e representa o conteúdo esperado ao final dessa etapa.}
...
- Explicação da responsabilidade:
  - Selecionar a próxima tarefa disponível na lista gerada pelo planejamento e submetê-la ao LLM para resolução.
  - Ao iniciar uma nova tarefa — e somente nesse momento —, o contexto é reinicializado. Os aprendizados pontuais acumulados nas tarefas anteriores são injetados no início do novo contexto, antes da descrição da tarefa atual, garantindo continuidade do conhecimento relevante sem carregar o ruído das execuções anteriores.
  - Na primeira tarefa do ciclo não há aprendizados acumulados; o contexto inicial contém apenas a tarefa a ser resolvida.

- Como vai ser feito:
  - Um template FIXO (Template B.1) orienta o modelo a focar exclusivamente no objetivo da tarefa atual, sem tentar antecipar ou resolver etapas posteriores. O contexto de entrada é montado da seguinte forma:
    - Aprendizados pontuais acumulados das tarefas anteriores (se houver)
    - Descrição completa da tarefa selecionada, conforme definida no planejamento
    - Template B.1

- Equação de funcionamento da fase:
  - Aprendizados Acumulados + Tarefa Selecionada + Template B.1 = Resultado Preliminar B

- Exemplo prático:
  - Considerando a \textbf{Tarefa 1} como primeira tarefa, o contexto não contém aprendizados acumulados. O LLM deveria retornar algo como:
...
% ————————————————————————
\subsubsection{Verificação dos critérios de aceite individuais} \label{subsec:ExecucaoB2}

- Explicação da responsabilidade:
  - Verificar se o Resultado Preliminar B produzido na etapa anterior atende a todos os critérios de aceite individuais associados à tarefa em execução.
  - Essa etapa funciona como um mecanismo de controle de qualidade interno: se todos os critérios forem atendidos, o fluxo avança para o armazenamento do resultado; caso contrário, a tarefa é reapresentada ao modelo para uma nova tentativa.
  - \textbf{Importante:} ao contrário do que ocorre na transição entre tarefas distintas, o contexto \emph{não} é reinicializado durante as tentativas de repetição de uma mesma tarefa. Essa escolha é intencional: a tentativa anterior, com os critérios não satisfeitos explicitamente sinalizados, permanece no contexto e auxilia o modelo a identificar o que deve ser corrigido na próxima execução.

- Como vai ser feito:
  - Um template FIXO (Template B.2) orienta o modelo a verificar, critério por critério, se o resultado atende ao esperado, retornando uma decisão estruturada (aprovado / reprovado) e, em caso de reprovação, identificando quais critérios não foram satisfeitos.

- Equação de funcionamento da fase:
  - Resultado Preliminar B + Critérios da Tarefa + Template B.2 = Decisão (Aprovado / Reprovado)

- Exemplo prático — Cenário de \textbf{aprovação} (critérios individuais atendidos):
  - Avaliando o resultado da Tarefa 1 contra seus critérios de aceite:

  \begin{quote}
  \begin{itemize}
    \item ``Deve retornar a lista de números primos encontrados nas alternativas.'' \\ \textit{Atendido}: a lista [2] foi retornada.
    \item ``Deve incluir uma breve explicação da regra de identificação de números primos.'' \\ \textit{Atendido}: a explicação por divisores foi fornecida.
  ...
  \end{itemize}
  Decisão: \textbf{Aprovado}. O fluxo avança para o armazenamento do resultado.
  \end{quote}

- Exemplo prático — Cenário de \textbf{reprovação} e nova tentativa (critérios individuais não atendidos):
  - Considere agora a \textbf{Tarefa 2} (``Dadas as opções (2, 4, 6), identifique quais desses números são pares''). Nesse ponto, o aprendizado acumulado da Tarefa 1 já está disponível no contexto: ``O único número primo entre as alternativas é o 2''. Suponha que o LLM, ao ter acesso a esse dado, produza na primeira tentativa:
  ...
  Avaliando esse resultado contra os critérios de aceite da Tarefa 2:

  \begin{quote}
  \begin{itemize}
    \item ``Deve retornar a lista de números pares encontrados nas alternativas.'' \\ \textit{Atendido}: os três números foram identificados como pares.
    \item ``Deve incluir uma breve explicação da regra de identificação de números pares.'' \\ \textit{Atendido}: a divisibilidade por 2 foi mencionada.
    \item ``A resposta não deve tentar resolver a questão final.'' \\ \textbf{Não atendido}: o modelo antecipou e apresentou a resposta final.
    \item ``A resposta não deve avaliar nenhuma outra condição.'' \\ \textbf{Não atendido}: o modelo cruzou condições pertencentes a outras tarefas.
  \end{itemize}
  Decisão: \textbf{Reprovado}. O fluxo retorna à resolução da Tarefa 2, mantendo o contexto atual — incluindo a tentativa reprovada e os critérios não satisfeitos identificados.
  \end{quote}

  Na segunda tentativa, com o feedback da reprovação anterior no contexto, o LLM deveria retornar algo como:
  ...
  Reavaliada essa resposta, todos os critérios da Tarefa 2 são agora atendidos. Decisão: \textbf{Aprovado}. O fluxo avança.
...
- Explicação da responsabilidade:
  - Ao ser aprovada, a resposta final da tarefa é persistida localmente. Somente essa resposta aprovada é armazenada: todo o ruído gerado pelos ciclos de tentativa anteriores — respostas reprovadas, histórico de verificações e feedback intermediário — não é persistido e será descartado quando o contexto for reinicializado na transição para a próxima tarefa.
  - O armazenamento é indexado por tarefa, de modo que cada resultado possa ser recuperado individualmente pela fase de Criação da Resposta Final.
  - \textbf{Esses resultados armazenados são o único insumo factual utilizado na fase C para a construção da resposta ao usuário.} Os aprendizados pontuais, tratados na etapa seguinte, têm escopo restrito à Execução em Loop e não participam da criação da resposta final.

- Como vai ser feito:
  - A resposta aprovada na etapa B.2 é persistida no armazenamento local ou na memória da aplicação, indexada pelo identificador da tarefa. O formato de armazenamento é uma decisão de implementação — qualquer estrutura que permita recuperar individualmente o resultado de cada tarefa é adequada. Não há chamada adicional ao LLM nessa etapa.

- Equação de funcionamento da fase:
  - Resposta Aprovada → Armazenamento local (formato estruturado)

- Exemplo prático:
  - A \textbf{Tarefa 2} exigiu duas tentativas antes de ser aprovada. Somente a segunda — aprovada — é armazenada. A estrutura abaixo usa o formato JSON a título ilustrativo; qualquer representação equivalente é válida:

  \begin{quote}
    \texttt{\{ "tarefa": 2, "resultado": "Entre as alternativas (2, 4, 6), todos os três números são pares, pois cada um é divisível por 2 sem deixar resto (2÷2=1, 4÷2=2, 6÷2=3). Resultado: [2, 4, 6]." \}}
  \end{quote}

  A primeira tentativa reprovada — aquela que antecipou indevidamente a resposta final — não é persistida. Permanece apenas no contexto ativo durante o ciclo de repetição e é descartada na reinicialização.

  Ao término da Execução em Loop, o armazenamento contém exclusivamente as respostas aprovadas de cada tarefa. Novamente usando JSON como exemplo de estrutura:

  \begin{quote}
  \begin{itemize}
    \item \texttt{\{ "tarefa": 1, "resultado": "O único número primo entre as alternativas (2, 4, 6) é o 2. Os números 4 e 6 possuem divisores além de 1 e eles mesmos." \}}
    \item \texttt{\{ "tarefa": 2, "resultado": "Entre as alternativas (2, 4, 6), todos os três números são pares: 2, 4 e 6." \}}
    \item \texttt{\{ "tarefa": 3, "resultado": "Os números estritamente maiores que 2 entre as alternativas são 4 e 6. O número 2 não satisfaz essa condição pois 2 > 2 é falso." \}}
  \end{itemize}
  \end{quote}

  Esse conjunto é entregue à fase C como base exclusiva para a construção da resposta final.
...
- Explicação da responsabilidade:
  - Os aprendizados pontuais são as ``anotações internas'' do fluxo de \textit{thinking}: fatos secos e objetivos descobertos durante a execução de uma tarefa que podem ser úteis para resolver as tarefas subsequentes.
  - Exemplos típicos incluem: resultado de um cálculo intermediário, uma fórmula identificada, uma conversão de unidade de medida, um valor recuperado após busca, uma lista de candidatos filtrados. Em todos os casos, o registro deve conter apenas a informação em si, sem qualquer contexto de como ela foi obtida.
  - A extração de aprendizados considera o contexto completo da tarefa — incluindo as tentativas reprovadas —, pois fatos úteis podem ter emergido mesmo em execuções que não passaram na verificação de critérios.
  - \textbf{Os aprendizados pontuais têm escopo estritamente restrito à Execução em Loop.} São injetados no início do contexto de cada nova tarefa para subsidiar a resolução, mas não são incluídos — em nenhuma hipótese — na fase de Criação da Resposta Final.

- Como vai ser feito:
  - Um template FIXO (Template B.4) é aplicado sobre o contexto completo da tarefa concluída para extrair os aprendizados pontuais. Esse template instrui o modelo a: (i) identificar fatos concretos que possam ser úteis nas próximas tarefas; (ii) registrá-los em formato de lista seca, sem narrativa ou contexto; e (iii) descartar todo o restante.
  - Os aprendizados extraídos são adicionados à memória acumulada, que será injetada no início do contexto da próxima tarefa.

- Equação de funcionamento da fase:
  - Contexto Completo da Tarefa (tentativas + resposta aprovada) + Template B.4 = Aprendizados Pontuais
  - Aprendizados Pontuais → Memória acumulada (injetada na próxima tarefa; não utilizada na resposta final)

- Exemplo prático:
  - Após a conclusão das três tarefas, a memória acumulada conteria algo como:
  ...
  Esse bloco compacto seria injetado no início do contexto de uma eventual Tarefa 4. Não é utilizado na construção da resposta final.

  Como contraponto, o trecho a seguir ilustra o formato \textbf{inválido} de aprendizado — verboso e com contexto desnecessário:
  ...
  \textit{Nota: a distinção entre resultado armazenado (etapa B.3) e aprendizado pontual (etapa B.4) é estrutural e intencional. O resultado é a resposta completa e verificada da tarefa, destinada à síntese da fase C. O aprendizado é um fragmento seco de fato, destinado exclusivamente a subsidiar a execução das tarefas seguintes dentro do loop.}
...
- Explicação da responsabilidade:
  - Reunir todos os resultados armazenados durante a Execução em Loop e verificar, se o conjunto de informações produzido é suficiente para satisfazer cada um dos critérios de aceite gerais definidos na etapa A.1.
  - A distinção central em relação à etapa B.2 é o escopo da verificação: enquanto lá a avaliação é local (cada tarefa verificada contra seus próprios critérios individuais), aqui a avaliação é global — o conjunto completo de resultados é considerado para determinar se é possível construir uma resposta final completa e correta.

- Como vai ser feito:
  - Um template FIXO (Template C.1) instrui o modelo a: (i) listar os resultados de cada tarefa; (ii) avaliar, critério geral por critério geral, se os resultados em conjunto permitem satisfazê-los; e (iii) retornar uma decisão estruturada (aprovado / reprovado) com identificação dos critérios não atendidos, quando for o caso.

- Equação de funcionamento da fase:
  - Resultados Armazenados + Critérios Gerais (Fase A.1) + Template C.1 = Decisão (Aprovado / Reprovado)

- Exemplo prático — Cenário de \textbf{aprovação} (critérios gerais atendidos):
  - Os resultados disponíveis ao término da Execução em Loop são:

  \begin{quote}
  \begin{itemize}
    \item T1: ``O único número primo entre as alternativas (2, 4, 6) é o 2.''
    \item T2: ``Todos os números entre as alternativas (2, 4, 6) são pares: 2, 4 e 6.''
    \item T3: ``Os números estritamente maiores que 2 entre as alternativas são 4 e 6.''
  \end{itemize}
  \end{quote}

  Avaliando esses resultados contra os critérios gerais definidos em A.1:

  \begin{quote}
  \begin{itemize}
    \item ``A pergunta deve ser respondida em Português do Brasil.'' \\ \textit{Atendível}: todos os resultados estão em português e permitem uma resposta no mesmo idioma.
    \item ``Responda somente a alternativa, sem conter explicações adicionais.'' \\ \textit{Atendível}: os dados disponíveis permitem identificar a alternativa correta sem necessidade de conteúdo adicional.
    \item ``Garanta que o valor encontrado ao final corresponde a uma das alternativas fornecidas.'' \\ \textit{Atendível}: a partir dos três resultados é possível concluir que nenhum número satisfaz simultaneamente as três condições (primo $\cap$ par $\cap$ maior que 2 = $\varnothing$), o que corresponde diretamente à alternativa D.
  \end{itemize}
  Decisão: \textbf{Aprovado}. O fluxo avança para a construção da resposta final.
  \end{quote}

- Exemplo prático — Cenário de \textbf{reprovação} (critérios gerais não atendidos):
  - Considere uma variação em que os critérios gerais definidos em A.1 incluam a seguinte exigência adicional:
    - ``A resposta deve ser sustentada por uma verificação explícita de qual conjunto de alternativas satisfaz simultaneamente todas as condições enunciadas.''
  - Os três resultados disponíveis cobrem cada condição individualmente, mas nenhuma tarefa realizou a análise de interseção. Ao tentar compilar, o modelo identifica que não há registro explícito de qual número — se algum — aparece simultaneamente nos três conjuntos:

  \begin{quote}
  \begin{itemize}
    \item ``A resposta deve ser sustentada por verificação explícita da interseção dos conjuntos.'' \\ \textbf{Não atendido}: nenhuma tarefa realizou a verificação combinada das três condições simultaneamente.
  \end{itemize}
  Decisão: \textbf{Reprovado}. O fluxo avança para o Planejamento Adicional.
  \end{quote}
...
- Explicação da responsabilidade:
  - Quando os critérios gerais não são atendidos, o Planejamento Adicional funciona como uma extensão da fase A.2: identifica as lacunas específicas nos resultados disponíveis e cria novas tarefas individuais para supri-las.
  - Diferentemente do planejamento inicial, que parte da pergunta do usuário sem qualquer contexto de execução, o Planejamento Adicional tem acesso ao conjunto de resultados já produzidos e ao mapeamento preciso dos critérios que ainda não foram satisfeitos. Essa informação torna as novas tarefas mais pontuais e direcionadas, evitando retrabalho sobre o que já foi verificado.
  - As novas tarefas são adicionadas à fila de execução e o fluxo retorna à etapa B.1 da Execução em Loop, agora com o contexto dos aprendizados acumulados de todas as tarefas anteriores disponível.

- Como vai ser feito:
  - Um template FIXO (Template C.2) orienta o modelo a: (i) identificar os critérios gerais não atendidos; (ii) raciocinar sobre que tipo de informação adicional seria necessária para satisfazê-los; e (iii) formular novas tarefas de objetivo único com critérios de aceite individuais, seguindo o mesmo padrão estabelecido na fase A.2.

- Equação de funcionamento da fase:
  - Critérios Gerais Não Atendidos + Resultados Armazenados + Template C.2 = Novas Tarefas
  - Novas Tarefas → adicionadas à fila → retorno à Execução em Loop (B.1)

- Exemplo prático:
  - A partir da reprovação identificada no cenário anterior, o Planejamento Adicional produziria algo como:
  ...
  Essa nova tarefa é enviada à Execução em Loop. O modelo, com os aprendizados acumulados de T1, T2 e T3 injetados no contexto, executa a Tarefa 4 e retorna algo como:

  \begin{quote}
    ``Verificando a interseção dos três conjuntos: primos [2] $\cap$ pares [2, 4, 6] $\cap$ maiores que 2 [4, 6].
    — O número 2 está em \{primos\} e em \{pares\}, mas não em \{maiores que 2\}, pois 2 > 2 é falso.
    — Os números 4 e 6 estão em \{pares\} e em \{maiores que 2\}, mas não em \{primos\}.
    Resultado da interseção: \{\} (conjunto vazio — nenhum número satisfaz simultaneamente as três condições).''
  \end{quote}

  Após a aprovação da Tarefa 4, o fluxo retorna à etapa C.1 para nova verificação. Desta vez, com quatro resultados disponíveis, todos os critérios gerais são atendidos e o fluxo avança para a construção da resposta final.

  \textit{Nota: o mecanismo de Planejamento Adicional pode ocorrer mais de uma vez caso as novas tarefas criadas ainda não sejam suficientes para satisfazer todos os critérios gerais. A cada retorno a C.1, o conjunto de resultados disponíveis é maior, convergindo progressivamente para a completude.}
...
- Explicação da responsabilidade:
  - Com todos os critérios gerais satisfeitos, o modelo compõe a resposta final ao usuário. Essa etapa não realiza novo raciocínio analítico sobre o problema: seu objetivo é exclusivamente organizar e apresentar os resultados já produzidos no formato e idioma definidos pelos critérios gerais (fase A.1).
  - A distinção entre esta etapa e as anteriores é deliberada: ao separar o raciocínio analítico — realizado de forma incremental e verificada durante a Execução em Loop — da síntese da resposta final, reduz-se o risco de que a fase de apresentação introduza inferências não verificadas ou contradiga os resultados produzidos.

- Como vai ser feito:
  - Um template FIXO (Template C.3) instrui o modelo a: (i) utilizar exclusivamente os resultados armazenados como base factual; (ii) respeitar o formato e o idioma definidos pelos critérios gerais; e (iii) não introduzir raciocínio ou informações novas que não estejam amparadas pelos resultados das tarefas.

- Equação de funcionamento da fase:
  - Resultados Compilados + Critérios Gerais (Fase A.1) + Template C.3 = Resposta Final

- Exemplo prático:
  - Considerando o cenário de aprovação direta (três tarefas), os resultados disponíveis indicam que primo $\cap$ par $\cap$ maior que 2 = $\varnothing$. Respeitando os critérios gerais — resposta em Português do Brasil, somente a alternativa, sem explicações adicionais, correspondente a uma das alternativas fornecidas —, a resposta final produzida seria:

  \begin{quote}
    \textbf{D) Nenhuma das anteriores}
  \end{quote}

  - Considerando o cenário com Planejamento Adicional (quatro tarefas), a Tarefa 4 produziu explicitamente a verificação da interseção vazia. Os critérios gerais, incluindo a exigência de verificação explícita, são plenamente satisfeitos, e a resposta final produzida é a mesma:
