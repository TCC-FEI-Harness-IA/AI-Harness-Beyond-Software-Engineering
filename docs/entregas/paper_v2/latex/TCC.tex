...
Para ilustrar concretamente cada etapa do fluxo, adota-se ao longo desta seção o seguinte \textit{input} de usuário como referência:
...
A implementação dessa etapa utiliza um \textit{template} fixo (Template~A.1), no qual o \textit{input} fornecido pelo usuário é injetado. Esse \textit{template} contém um \textit{prompt} que solicita ao modelo a definição de uma lista de critérios de aceite, os quais serão resgatados posteriormente na fase de Criação da Resposta Final. Especificamente, o \textit{template} solicita a definição do idioma a ser usado na resposta final, do formato em que a resposta deve ser construída e dos tópicos-chave do conteúdo que devem estar presentes. A relação de entrada e saída dessa etapa pode ser expressa como:
...
O resultado produzido é armazenado localmente em arquivo ou na memória do programa.

\begin{exemplo}
Considerando o \textit{input} de referência, o modelo deveria retornar critérios como: ``A pergunta deve ser respondida em Português do Brasil''; ``Responda somente a alternativa, sem conter explicações adicionais''; e ``Garanta que o valor encontrado ao final corresponde a uma das alternativas fornecidas''.
\end{exemplo}
...
A implementação utiliza um \textit{template} fixo (Template~A.2), no qual o \textit{input} do usuário é injetado e a quebra em tarefas é solicitada. Esse \textit{template} especifica que tarefas de objetivo único possuem uma única responsabilidade pontual, como ``identificar os números primos'' ou ``identificar os números pares'', e nunca múltiplas responsabilidades simultâneas. Além de ter um único objetivo, cada tarefa deve ser descrita da forma mais detalhada possível, de modo a evitar desvios durante a resolução. O \textit{template} também torna explícito que o contexto de execução de uma tarefa não é transferido para a próxima, já que somente pontos específicos sintetizados como aprendizados pontuais serão preservados. Por fim, toda tarefa deve conter pelo menos um critério de aceite que valide se a resposta gerada contém o conteúdo necessário para encerrá-la. A relação de entrada e saída pode ser expressa como:
...
\begin{exemplo}
Considerando o \textit{input} de referência, o modelo deveria retornar algo como:
...
Essa etapa é responsável por selecionar a próxima tarefa disponível na lista gerada pelo planejamento e submetê-la ao modelo para resolução. Ao iniciar uma nova tarefa, e somente nesse momento, o contexto é reinicializado. Os aprendizados pontuais acumulados nas tarefas anteriores são injetados no início do novo contexto, antes da descrição da tarefa atual, garantindo continuidade do conhecimento relevante sem carregar o ruído das execuções anteriores. Na primeira tarefa do ciclo, não há aprendizados acumulados, de modo que o contexto inicial contém apenas a tarefa a ser resolvida.

A implementação utiliza um \textit{template} fixo (Template~B.1) que orienta o modelo a focar exclusivamente no objetivo da tarefa atual, sem tentar antecipar ou resolver etapas posteriores. O contexto de entrada é montado a partir de três componentes: os aprendizados pontuais acumulados das tarefas anteriores (quando houver), a descrição completa da tarefa selecionada conforme definida no planejamento, e o próprio Template~B.1. A relação de entrada e saída pode ser expressa como:

\begin{equation*}
  \begin{aligned}
    	ext{Aprendizados Acumulados} + \text{Tarefa Selecionada} \\
    + \text{Template B.1} \longrightarrow \text{Resultado Preliminar B}
  \end{aligned}
\end{equation*}

\begin{exemplo}
Considerando a Tarefa~1 como primeira tarefa do ciclo, o contexto não contém aprendizados acumulados. O modelo deveria retornar algo como:
...
A implementação utiliza um \textit{template} fixo (Template~B.2) que orienta o modelo a verificar, critério por critério, se o resultado atende ao esperado, retornando uma decisão estruturada (aprovado ou reprovado) e, em caso de reprovação, identificando quais critérios não foram satisfeitos. A relação de entrada e saída pode ser expressa como:

\begin{equation*}
  \begin{aligned}
    	ext{Resultado Preliminar B} + \text{Critérios da Tarefa} \\
    + \text{Template B.2} \longrightarrow \text{Decisão}
  \end{aligned}
\end{equation*}
...
\end{itemize}
Decisão: \textbf{Reprovado}. O fluxo retorna à resolução da Tarefa~2, mantendo o contexto com a tentativa reprovada e os critérios não satisfeitos identificados.

\smallskip
Na segunda tentativa, com o \textit{feedback} anterior no contexto, o modelo retorna:
...
\begin{exemplo}
A Tarefa~2 exigiu duas tentativas antes de ser aprovada. Somente a segunda é armazenada. A estrutura abaixo utiliza JSON a título ilustrativo; qualquer representação equivalente é válida:

\smallskip
\texttt{\{ "tarefa": 2, "resultado": "Entre as alternativas (2,~4,~6), todos os três números são pares, pois cada um é divisível por 2 sem deixar resto. Resultado: [2, 4, 6]." \}}

\smallskip
A primeira tentativa reprovada não é persistida. Ao término da Execução em Loop, o armazenamento contém exclusivamente as respostas aprovadas de cada tarefa:

\begin{itemize}
  \item \texttt{\{ "tarefa": 1, "resultado": "O único número primo entre (2, 4, 6) é o 2. Os números 4 e 6 possuem divisores além de 1 e eles mesmos." \}}
  \item \texttt{\{ "tarefa": 2, "resultado": "Entre (2, 4, 6), todos os três números são pares: 2, 4 e 6." \}}
...
A implementação utiliza um \textit{template} fixo (Template~B.4) aplicado sobre o contexto completo da tarefa concluída. Esse \textit{template} instrui o modelo a identificar fatos concretos que possam ser úteis nas próximas tarefas, registrá-los em formato de lista seca sem narrativa ou contexto, e descartar todo o restante. Os aprendizados extraídos são adicionados à memória acumulada, que será injetada no início do contexto da próxima tarefa. A relação de entrada e saída pode ser expressa como:

\begin{equation*}
  \begin{aligned}
    	ext{Contexto Completo da Tarefa} + \text{Template B.4} \\
    \longrightarrow \text{Aprendizados Pontuais} \rightarrow \text{Memória Acumulada}
  \end{aligned}
\end{equation*}
...
A distinção entre resultado armazenado (etapa~B.3) e aprendizado pontual (etapa~B.4) é estrutural e intencional. O resultado é a resposta completa e verificada da tarefa, destinada à síntese da fase~C. O aprendizado é um fragmento seco de fato, destinado exclusivamente a subsidiar a execução das tarefas seguintes dentro do loop.
...
Essa etapa reúne todos os resultados armazenados durante a Execução em Loop e verifica se o conjunto de informações produzido é suficiente para satisfazer cada um dos critérios de aceite gerais definidos na etapa~A.1. A distinção central em relação à etapa~B.2 é o escopo da verificação: enquanto lá a avaliação é local, com cada tarefa verificada contra seus próprios critérios individuais, aqui a avaliação é global, e o conjunto completo de resultados é considerado para determinar se é possível construir uma resposta final completa e correta.

A implementação utiliza um \textit{template} fixo (Template~C.1) que instrui o modelo a listar os resultados de cada tarefa, avaliar critério geral por critério geral se os resultados em conjunto permitem satisfazê-los, e retornar uma decisão estruturada com identificação dos critérios não atendidos, quando for o caso. A relação de entrada e saída pode ser expressa como:

\begin{equation*}
  \begin{aligned}
    	ext{Resultados Armazenados} + \text{Critérios Gerais (A.1)} \\
    + \text{Template C.1} \longrightarrow \text{Decisão}
  \end{aligned}
\end{equation*}

\begin{exemplo}
\textbf{Cenário de aprovação.} Com os três resultados disponíveis ao término do loop (T1: o único primo é [2]; T2: todos os números são pares [2, 4, 6]; T3: os maiores que 2 são [4, 6]), a avaliação contra os critérios gerais de A.1 produz:
...
A implementação utiliza um \textit{template} fixo (Template~C.2) que orienta o modelo a identificar os critérios gerais não atendidos, raciocinar sobre que tipo de informação adicional seria necessária para satisfazê-los, e formular novas tarefas de objetivo único com critérios de aceite individuais, seguindo o mesmo padrão estabelecido na fase~A.2. A relação de entrada e saída pode ser expressa como:

\begin{equation*}
  \begin{aligned}
    	ext{Critérios Não Atendidos} + \text{Resultados Armazenados} \\
    + \text{Template C.2} \longrightarrow \text{Novas Tarefas}
  \end{aligned}
\end{equation*}
...
Com os aprendizados de T1, T2 e T3 injetados no contexto, o modelo executa a Tarefa~4 e retorna algo como:

\smallskip
``Verificando a interseção: primos [2] $\cap$ pares [2, 4, 6] $\cap$ maiores que 2 [4, 6]. O número 2 está em \{primos\} e \{pares\}, mas não em \{maiores que 2\} (pois $2 > 2$ é falso). Os números 4 e 6 estão em \{pares\} e \{maiores que 2\}, mas não em \{primos\}. Resultado da interseção: $\{\}$ (conjunto vazio).''

\smallskip
Após a aprovação da Tarefa~4, o fluxo retorna a C.1. Com quatro resultados disponíveis, todos os critérios gerais são atendidos e o fluxo avança para a construção da resposta final.
\end{exemplo}

O mecanismo de Planejamento Adicional pode ocorrer mais de uma vez caso as novas tarefas criadas ainda não sejam suficientes para satisfazer todos os critérios gerais. A cada retorno a C.1, o conjunto de resultados disponíveis é maior, convergindo progressivamente para a completude.


% ————————————————————————
\subsubsection{Construção da resposta final} \label{subsec:RespostaC3}

Com todos os critérios gerais satisfeitos, o modelo compõe a resposta final ao usuário. Essa etapa não realiza novo raciocínio analítico sobre o problema: seu objetivo é exclusivamente organizar e apresentar os resultados já produzidos no formato e idioma definidos pelos critérios gerais (fase~A.1). A distinção entre esta etapa e as anteriores é deliberada: ao separar o raciocínio analítico, realizado de forma incremental e verificada durante a Execução em Loop, da síntese da resposta final, reduz-se o risco de que a fase de apresentação introduza inferências não verificadas ou contradiga os resultados produzidos.

A implementação utiliza um \textit{template} fixo (Template~C.3) que instrui o modelo a utilizar exclusivamente os resultados armazenados como base factual, respeitar o formato e o idioma definidos pelos critérios gerais, e não introduzir raciocínio ou informações novas que não estejam amparadas pelos resultados das tarefas. A relação de entrada e saída pode ser expressa como:

\begin{equation*}
  \begin{aligned}
    	ext{Resultados Compilados} + \text{Critérios Gerais (A.1)} \\
    + \text{Template C.3} \longrightarrow \text{Resposta Final}
  \end{aligned}
\end{equation*}

\begin{exemplo}
Em ambos os cenários (aprovação direta com três tarefas ou com Planejamento Adicional em quatro tarefas), os resultados indicam que primo $\cap$ par $\cap$ maior que 2 = $\varnothing$. Respeitando os critérios gerais — resposta em Português do Brasil, somente a alternativa, sem explicações adicionais, correspondente a uma das alternativas fornecidas —, a resposta final produzida é:
